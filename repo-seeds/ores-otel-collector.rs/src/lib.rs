...
use axum::{
    ...
    http::{header::{CONTENT_ENCODING, CONTENT_TYPE}, HeaderMap, StatusCode},
...
impl Config {
    pub fn from_env() -> Result<Self, ConfigError> {
        ...
        let upstream_raw = env::var("ORES_OTEL_UPSTREAM_URL").map_err(|_| ConfigError::InvalidUpstream)?;
        let upstream_base = validate_upstream(&upstream_raw)?;
        let internal_auth = env::var("ORES_OTEL_INTERNAL_AUTH").ok().filter(|v| !v.is_empty());
        if !bind.ip().is_loopback() && internal_auth.is_none() {
            return Err(ConfigError::MissingAuth);
        }
        let max_body_bytes = parse_bounded_env("ORES_OTEL_MAX_BODY_BYTES", DEFAULT_MAX_BODY_BYTES, 1, HARD_MAX_BODY_BYTES)?;
        let max_concurrent = parse_bounded_env("ORES_OTEL_MAX_CONCURRENT", DEFAULT_MAX_CONCURRENT, 1, 4096)?;
        Ok(Self { bind, upstream_base, internal_auth, max_body_bytes, max_concurrent })
    }
}

fn parse_bounded_env(name: &'static str, default: usize, min: usize, max: usize) -> Result<usize, ConfigError> {
    let Some(raw) = env::var(name).ok() else { return Ok(default); };
    let parsed = raw.parse::<usize>().map_err(|_| ConfigError::InvalidLimit(name))?;
...
fn is_loopback_host(url: &Url) -> bool {
    match url.host_str() {
        Some("localhost") => true,
        Some(host) => host.parse::<IpAddr>().map(|ip| ip.is_loopback()).unwrap_or(false),
        None => false,
    }
}
...
async fn ingest(
    ...
    let content_type = match headers.get(CONTENT_TYPE).and_then(|v| v.to_str().ok()) {
        Some(value) if value.starts_with("application/x-protobuf") || value.starts_with("application/json") => value.to_owned(),
        _ => return error(StatusCode::UNSUPPORTED_MEDIA_TYPE, "unsupported_content_type"),
    };
    if headers.get(CONTENT_ENCODING).is_some() {
        return error(StatusCode::UNSUPPORTED_MEDIA_TYPE, "compressed_otlp_not_yet_supported");
    }
    ...
    let upstream = match state.config.upstream_base.join(uri.path().trim_start_matches('/')) {
        Ok(url) => url,
    ...
    builder.body(Body::from(response_bytes)).unwrap_or_else(|_| error(StatusCode::BAD_GATEWAY, "response_build_failed"))
}
...
fn required_identity(headers: &HeaderMap, name: &'static str) -> Result<String, Response> {
    ...
    if value.is_empty() || value.len() > 128 || !value.bytes().all(|b| b.is_ascii_alphanumeric() || matches!(b, b'-' | b'_' | b'.' | b':')) {
...
mod tests {
    ...
    fn authenticated_scope_is_header_authoritative() {
        ...
        assert_eq!(authorize(&config(Some("secret")), &headers).unwrap(), ("tenant_1".into(), "api_1".into()));
    }
